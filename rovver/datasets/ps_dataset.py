...
from rovver.datasets.base import BaseDataset
from rovver.datasets.builder import DATASETS
from rovver.utils.precision_recall import calc_average_precision, calc_precision_recall

from rovver.utils.pre_process import boundary_check, overlap_check, rotate_centralized_marks, rotate_image, generalize_marks, match_marking_points, match_slots 
...
class ParkingSlotDataset(BaseDataset):
    ...
    def __init__(self, cfg, logger=None):
        ...
        if cfg.mode == 'train': 
            # data augmentation T.ToTensor() also normalize [0,1]
    ...
    def __getitem__(self, idx):
        ...
        img_file = str(self.json_files[idx]).replace('.json', '.jpg').replace('json', 'imgs') #.replace('json', 'train') #
