...
PredMarkingPoint = namedtuple('PredMarkingPoint', ['x', 'y', 'direction', 'shape', 'x_third', 'y_third'])
...
def detemine_point_shape(point, vector, img_name):
    """Determine which category the point is in."""
    # [point_a, vector_ab] or  [point_b, vector_ba] 
    vec_direct = math.atan2(vector[1], vector[0])       # 入口线向量 角度
    vec_direct_up = math.atan2(-vector[0], vector[1])   # 逆时针垂直的法向量 角度 
    vec_direct_down = math.atan2(vector[0], -vector[1]) # 顺时针垂直的法向量 角度
    if point.shape < 0.5:
        if direction_diff(vec_direct, point.direction) < config.BRIDGE_ANGLE_DIFF:
            print(f"img {img_name}, point_{point} and vector_{vector} is t_middle")
            return PointShape.t_middle
        if direction_diff(vec_direct_up, point.direction) < config.SEPARATOR_ANGLE_DIFF:
            print(f"img {img_name}, point_{point} and vector_{vector} is t_up")
            return PointShape.t_up
        if direction_diff(vec_direct_down, point.direction) < config.SEPARATOR_ANGLE_DIFF:
            print(f"img {img_name}, point_{point} and vector_{vector} is t_down")
            return PointShape.t_down
    else:
        if direction_diff(vec_direct, point.direction) < config.BRIDGE_ANGLE_DIFF:
            print(f"img {img_name}, point_{point} and vector_{vector} is l_down")
            return PointShape.l_down
        if direction_diff(vec_direct_up, point.direction) < config.SEPARATOR_ANGLE_DIFF:
            print(f"img {img_name}, point_{point} and vector_{vector} is l_up")
            return PointShape.l_up
    print(f"img {img_name}, point_{point} and vector_{vector} is none")
...
def calc_point_squre_dist(point_a, point_b):
    ...
    # ['x', 'y', 'direction', 'shape', 'x_third', 'y_third']
