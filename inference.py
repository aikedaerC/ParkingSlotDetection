...
def detect_image(detector, device, args):
    ...
    if args.interactive:
        ...
        from compare_nonslot import dp_detector_24
        # imglist = os.listdir(args.imgsPath)
        imglist = list(dp_detector_24)
        outputpath = args.imgsPath.replace("imgs", "pred")
        os.makedirs(outputpath, exist_ok=True)
        for f in tqdm(imglist):
            fpath = os.path.join(args.imgsPath, f)
            ...
            cv.waitKey(900)
            if args.save:
                fopath = os.path.join(outputpath, f)
                cv.imwrite(fopath, image, [int(cv.IMWRITE_JPEG_QUALITY), 100])
...
# python inference.py --mode image --inference_slot --detector_weights F:\ubunut_desktop_back\ParkingSlotDetection\NEW_DMPRbakup\weights\dp_detector_24.pth 
# --imgsPath F:\ubunut_desktop_back\ParkingSlotDetection\data_comp\train\imgs