...
class DirectionalPointDetector(nn.modules.Module):
    ...
    def __init__(self, input_channel_size, depth_factor, output_channel_size):
        ...
        # self.conv1x1 = nn.Conv2d(1, 4, 1, 1, 0)
    ...
    def forward(self, *x):
        ...
        point_pred, dirt_pred, third_point_pred = torch.split(prediction, [4, 2, 3], dim=1)
        point_pred = torch.sigmoid(point_pred)
        dirt_pred = torch.tanh(dirt_pred)
        third_point_pred = torch.sigmoid(third_point_pred)
        return torch.cat((point_pred, dirt_pred, third_point_pred), dim=1)
