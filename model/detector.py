...
class DirectionalPointDetector(nn.modules.Module):
    ...
    def forward(self, *x):
        ...
        # offset_y, whose range is between [0, 1] guaranteed by sigmoid.
        point_pred, angle_pred = torch.split(prediction, 4, dim=1)
        point_pred = torch.sigmoid(point_pred) # scale to [0,1]
        angle_pred = torch.tanh(angle_pred) # scale to [-1,1]
        return torch.cat((point_pred, angle_pred), dim=1)

# input label is : MarkingPoint = namedtuple('MarkingPoint', ['x', 'y', 'direction', 'shape', 'angle'])
# pred label is : confidence, shape, offset_x, offset_y, cos_value, sin_value